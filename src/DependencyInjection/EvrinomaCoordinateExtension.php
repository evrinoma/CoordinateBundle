...
use Evrinoma\CoordinateBundle\Factory\Coordinate\Factory as CoordinateFactory;
use Evrinoma\CoordinateBundle\Mediator\QueryMediatorInterface;
use Evrinoma\CoordinateBundle\Repository\Coordinate\CoordinateCommandRepositoryInterface;
...
class EvrinomaCoordinateExtension extends Extension
{
    use HelperTrait;

    public const ENTITY = 'Evrinoma\CoordinateBundle\Entity';
    public const MODEL = 'Evrinoma\CoordinateBundle\Model';
    public const ENTITY_FACTORY_COORDINATE = CoordinateFactory::class;
    public const ENTITY_BASE_COORDINATE = BaseCoordinate::class;
    public const DTO_BASE_COORDINATE = CoordinateApiDto::class;
    ...
    public function load(array $configs, ContainerBuilder $container)
    {
        ...
        if (self::ENTITY_FACTORY_COORDINATE !== $config['factory']) {
    ...
    private function wireConstraintTag(ContainerBuilder $container): void
    {
        foreach ($container->getDefinitions() as $key => $definition) {
            switch (true) {
                case str_contains($key, CoordinatePass::COORDINATE_CONSTRAINT)   :
                    $definition->addTag(CoordinatePass::COORDINATE_CONSTRAINT);
                    break;
                default:
            }
        }
    ...
    private function wireRepository(ContainerBuilder $container, Reference $registry, string $madiator, string $class, string $driver): void
    {
        ...
        $container->addAliases([CoordinateCommandRepositoryInterface::class => 'evrinoma.'.$this->getAlias().'.repository']);
        $container->addAliases([CoordinateQueryRepositoryInterface::class => 'evrinoma.'.$this->getAlias().'.repository']);
    }
    ...
    {
        return EvrinomaCoordinateBundle::BUNDLE;
    }
}
